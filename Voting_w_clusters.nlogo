...
globals [data-points colored-patches]

to setup
  clear-all
  ask patches
  [
    set pcolor blue  ; Set the default color to blue
    set vote 1       ; Set the default vote to 1
  ]
  set colored-patches 0
  generate-clusters
  reset-ticks
end

to generate-clusters
  let num-data-points 11300 + random (11500 - 11300)
  set data-points num-data-points / num-clusters
  repeat num-clusters [
    let center-patch one-of patches with [abs pxcor < max-pxcor / (1 + 0.5 * world-width / 301) and abs pycor < max-pycor / (1 + 0.5 * world-height / 301)]
    let nearby-patches patches with [distance center-patch <= (cluster-std-dev / 2 * min list (world-width / 301) (world-height / 301)) and pcolor = blue]  ; Only consider patches that are not yet colored.
    let patches-to-color min (list data-points count nearby-patches)
    if patches-to-color + colored-patches > num-data-points [  ; Ensure that we don't color more patches than num-data-points.
      set patches-to-color num-data-points - colored-patches
    ]
    ask n-of patches-to-color nearby-patches [
      set pcolor green
      set vote 0  ; Set the vote of the green patches to 0
    ]
    set colored-patches colored-patches + patches-to-color
  ]
  if colored-patches < num-data-points [  ; If we have not yet colored enough patches, color some more at random.
    ask n-of (num-data-points - colored-patches) patches with [pcolor = blue] [
      set pcolor green
      set vote 0  ; Set the vote of the green patches to 0
    ]
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="experiment-1-30May2023" repetitions="250" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>count patches with [pcolour = green]</metric>
    ...
    <enumeratedValueSet variable="cluster-std-dev">
      <value value="1"/>
      <value value="75"/>
      <value value="150"/>
      <value value="225"/>
    ...
    <steppedValueSet variable="num-clusters" first="1" step="1" last="10"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
